...
- `src/data/fleet.ts` — 9 FleetCategory entries (charter-buses removed 6 Aug 2026, see "Charter Buses category removed" below); images point to `public/fleet/*.png` (kebab-case, renamed from originals). Featured trio on home + city pages: motor-coaches, mini-buses, sprinter-vans.
...
- `src/data/network/network-map.json` (21 KB) + `public/brand/network-map.svg` (animated) + `public/brand/network-map-still.svg` — the nationwide operations map on `/locations`. **All three are generated, never hand-edited:** `npm run network-map` (`scripts/generate-network-map.mjs`) renders the owner's fixed roster of **114 hubs (40 company / 74 partner)** and writes them. See "Operations network map" below.
...
- Output dimensions now equal the pristine source exactly, which is the cheap regression test: `public/fleet/*.png` dims must match `assets/fleet-source/*.png`. Any inequality means padding crept back.
...
- **`/fleet/charter-bus-exterior.png` and `-interior.png` must stay in `assets/fleet-source/`** — the category is gone but the home-hero fallback (`hero-media.tsx`) still uses the exterior.
...
Marketing visualisation of the existing 16,399-city directory, shown as the owner's fixed roster of 114 hubs. It does **not** replace the location pages — every hub links to its real city page, and the full state/city browser still sits below it.
...
- Consequences of the fixed roster, accepted as owner intent: **Mississippi is the one state with no dot** (the 6 Aug revision added hubs in ME, NH, VT, WV, MT, WY, ND, SD; a 16-hub gap-fill pass was applied same day and then removed at the owner's request — the roster is exactly the supplied 114), and city coverage within 100 mi of a hub is 84.9% (was ~100% of 45k+ markets under the algorithmic roster). The map description does not claim all-states coverage.
...
- Label placement tries 8 positions per hub and rejects collisions against other labels, every hub dot, and the AK/HI inset panels. **103 of 114 currently place; all 40 company hubs are labelled.** Unlabelled (all partner): Jersey City, Anaheim, Santa Ana, Riverside, Santa Barbara, Ann Arbor, Chapel Hill, Knoxville, Pigeon Forge, Asheville, State College — all boxed in by dense clusters the owner's roster creates (six hubs in the LA basin; Gatlinburg/Pigeon Forge are 5 mi apart; Jersey City is 2 mi from the NYC dot). Fixing these needs leader lines or abbreviated labels, not more placement candidates.
