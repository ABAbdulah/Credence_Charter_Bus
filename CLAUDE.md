...
- `src/data/network/network-map.json` (23 KB) + `public/brand/network-map.svg` (animated) + `public/brand/network-map-still.svg` — the nationwide operations map on `/locations`. **All three are generated, never hand-edited:** `npm run network-map` (`scripts/generate-network-map.mjs`) renders the owner's fixed roster of **130 hubs (40 company / 90 partner)** and writes them. See "Operations network map" below.
...
Marketing visualisation of the existing 16,399-city directory, shown as the owner's fixed roster of 130 hubs. It does **not** replace the location pages — every hub links to its real city page, and the full state/city browser still sits below it.
...
- **Every state now has at least one dot** (the 6 Aug gap-fill added Jackson + Biloxi, the last hubless state was Mississippi), and city coverage within 100 mi of a hub is 89.0% (was ~100% of 45k+ markets under the algorithmic roster). The map description does not claim all-states coverage.
...
- Label placement tries 8 positions per hub and rejects collisions against other labels, every hub dot, and the AK/HI inset panels. **119 of 130 currently place; all 40 company hubs are labelled.** Unlabelled (all partner): Jersey City, Anaheim, Santa Ana, Riverside, Santa Barbara, Ann Arbor, Chapel Hill, Knoxville, Pigeon Forge, Asheville, State College — all boxed in by dense clusters the owner's roster creates (six hubs in the LA basin; Gatlinburg/Pigeon Forge are 5 mi apart; Jersey City is 2 mi from the NYC dot). Fixing these needs leader lines or abbreviated labels, not more placement candidates.
