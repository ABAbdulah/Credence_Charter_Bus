...
- `src/data/network/network-map.json` (27 KB) + `public/brand/network-map.svg` (animated) + `public/brand/network-map-still.svg` — the nationwide operations map on `/locations`. **All three are generated, never hand-edited:** `npm run network-map` (`scripts/generate-network-map.mjs`) clusters the 16,399 cities into **146 hubs (70 company / 76 partner)** and writes them. See "Operations network map" below.
...
Marketing visualisation of the existing 16,399-city directory rolled up into 150 regional hubs. It does **not** replace the location pages — every hub links to its real city page, and the full state/city browser still sits below it.
...
### Clustering rules that matter (all in `scripts/generate-network-map.mjs`)
- **Candidates are real markets only — `MIN_MARKET_POPULATION` is 45,000.** This is the rule that makes the map believable, and it was learned the hard way: the old floor let the gap-fill reach into 3,000-person towns and shipped hubs at Shelby MT (3,268), Ely MN (3,408), Presidio TX (3,904), Presque Isle ME (9,171) and Pierre SD (14,091). The owner rejected it on sight — a reader who knows there is no bus company in Shelby stops believing the other 145 dots. Coverage gaps in empty country are honest; invented hubs are not. Cross-checked against a real national operator's published location list, which names none of those towns.
- Two exception lists, both small and explicit. `REGIONAL_MARKETS` lets a sub-floor city in when it is the transport centre of a region with no larger city near it (Burlington VT, Bangor ME, Bozeman MT, Fairbanks AK, Hilo HI) — without them their state or island has no reachable hub. `PRINCIPAL_MARKETS` seeds a city ahead of the population ranking so separation blocks its neighbours instead of it: **San Jose (997k) was displacing San Francisco (828k) 42 mi away**, putting the wrong name on the Bay Area's dot. Ranking by metro instead would hand it to Hayward, whose 25-mile rollup (5.5M) is the largest in the region — which city a reader expects is a recognition judgement, not a population fact, so it stays a curated list.
- Two-phase selection: **108 anchor hubs** ranked by own population with a metro-density-dependent minimum separation (48 mi in dense metros → 155 mi rural), then a **gap-fill pass** that repeatedly adds the best city more than 100 mi from any hub. With the market floor in place the fill exhausts its candidates before the 150 budget, which is why the map settles at **146 hubs** — the floor is the binding constraint now, not `TARGET_HUBS`. Useful invariant: every US city of 45k+ is within 100 mi of a hub (worst case Harrisonburg VA, 99 mi).
- **One hub per state is guaranteed** between the two phases. Without it DE, MD, NH, NJ and RI showed no dot at all — a visitor in New Jersey seeing an empty state defeats the whole point.
- `snapToLocalAnchor` (15 mi) fixes the suburb-instead-of-city failure: separation thresholds are knife-edge, so Chattanooga missed by 3 mi while its inflated CDP "East Chattanooga" passed by 1. Any hub is snapped to the largest untaken city within 15 miles.
- Military installations are filtered out by name (`Ellsworth Air Force Base` was beating Rapid City).
- **`locations.json` has a bad record: `kentucky/meads` reports population 288,649 for a ~7k CDP.** It is excluded via `BAD_POPULATION` so Huntington, WV anchors the Tri-State. The bad value still affects `/locations/kentucky/meads` itself — fix the dataset and drop the exclusion.
...
- A steep ring falloff turned 146 overlapping discs into one solid blue field with no readable service areas; that is why the gradients stay near-flat.
- Label placement tries 8 positions per hub and rejects collisions against other labels, every hub dot, and the AK/HI inset panels. 144 of 146 currently place; **San Francisco and Baltimore render as dots with no name**. Both are boxed in by neighbouring hub dots, not by the canvas edge — SF's 66-unit-wide name cannot fit left (off-canvas), centred (off-canvas), or right (Sacramento and Modesto dots). Ordering labels by weight alone instead of company-first was tried and is worse: it wins Baltimore but loses Springfield MA and Toledo (143 placed). Fixing these two needs leader lines or abbreviated labels, not more placement candidates.

### Animation — and the reduced-motion trap
Company hubs blink (2.6 s) with an expanding "ping" halo; partner service areas pulse in opacity (8 s) while the shared `#ring-partner` gradient slowly shifts `stop-color` (11 s). All delays are **staggered by longitude** (`wave()`), so it reads as one slow wave crossing the country instead of 150 independent flickers. Costs ~6.5% of one core on desktop; well under the WCAG three-flashes-per-second threshold.
...
### Company/partner split — owner-directed, overrides the original brief
The written brief asked for 20–30 company hubs and "clear visual dominance of Partner hubs". The owner has since raised it twice, landing on **70 company / 80 partner**. That inversion is deliberate; do not "fix" it back.
- **Company rank is the hub's own metro population, NOT its assigned catchment.** Catchment is a Voronoi share, so it rewards isolation rather than market size and produced a straight inversion: Peoria, Green Bay and Cedar Rapids came out company-owned while **Boise, Tulsa, Des Moines, Wichita and Spokane came out partner**. The owner called this out for Boise specifically. Metro ranking fixes it — company is now the 70 largest markets, top to bottom. The same `weight` field also drives label priority and JSON ordering, so all three follow one notion of importance.
- `COMPANY_MIN_SEPARATION_MILES` is **55**, not the earlier 150. At 85+ the last company slots fell to rural gap-fillers (Ely, Rexburg, Page, Garden City) while **Philadelphia and Baltimore were pushed to partner** — a top-10 metro reading as "partner" while Waco reads as "company-owned" is not credible. At 55 the split lands correctly: company = the 70 largest markets, partner = satellite cities blocked by separation (Newark, Baltimore, Riverside, Providence) plus every rural anchor. The underlying hub set is already ≥48 mi apart, so 55 only prevents two red dots from touching. Do not lower it to buy Baltimore a red dot: admitting Baltimore, Riverside, Bridgeport and Providence evicts the four smallest company hubs, and **Boise is one of them**.

### Palette: red company hubs, and the green the references wanted
- **Company hubs are `--map-company: #a82c31`** (deep crimson), dot radius 5 vs the partner 3.4, so they read first. Chosen over a pure red because the design system bans saturated red; over `--destructive` because that token stays semantic for error states. Their coverage **rings stay navy** — translucent red over the blue partner field turns purple where they overlap, and the coverage wash has to stay one coherent colour. Hub *labels* also stay navy: 70 red labels at 9.6px would read as alarm, not emphasis.
