...
- `src/data/network/network-map.json` (18 KB) + `public/brand/network-map.svg` (animated) + `public/brand/network-map-still.svg` — the nationwide operations map on `/locations`. **All three are generated, never hand-edited:** `npm run network-map` (`scripts/generate-network-map.mjs`) renders the owner's fixed roster of **99 hubs (40 company / 59 partner)** and writes them. See "Operations network map" below.
...
Marketing visualisation of the existing 16,399-city directory, shown as the owner's fixed roster of 99 hubs. It does **not** replace the location pages — every hub links to its real city page, and the full state/city browser still sits below it.
...
- Consequences of the fixed roster, accepted as owner intent: **nine states have no dot** (ME, NH, VT, WV, MS, MT, WY, ND, SD), and city coverage within 100 mi of a hub is 81.7% (was ~100% of 45k+ markets). The map description no longer claims all-states coverage.
...
- Label placement tries 8 positions per hub and rejects collisions against other labels, every hub dot, and the AK/HI inset panels. **93 of 99 currently place.** Unlabelled: San Francisco and Anaheim (company), Jersey City, Santa Ana, Hartford, Toledo (partner) — all boxed in by dense clusters the owner's roster creates (six hubs in the LA basin; Jersey City is 2 mi from the NYC dot). Fixing these needs leader lines or abbreviated labels, not more placement candidates.
...
### Company/partner split — owner-fixed, third revision
The written brief asked for 20–30 company hubs; the owner revised the split twice (landing at 70/76 algorithmic) and then, on 5 Aug 2026, replaced selection entirely with the explicit 40-company / 59-partner roster now in the generator. The split is part of the supplied list, not derived — Boise, Tulsa, Des Moines, Wichita and Spokane are partner *by owner instruction this time*, and satellite cities the algorithm once demoted (Newark, Baltimore, Riverside) are company. Do not "correct" any of it without the owner.
