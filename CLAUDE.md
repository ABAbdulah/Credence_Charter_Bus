...
`src/hooks/use-form-submission.ts` holds the state machine all six forms used to duplicate (data/errors/status/honeypot state, `set`/`ariaProps`/`fieldProps` factories, validate → focus first invalid → POST → status). `src/components/site/form-status.tsx` holds `FormSuccessCard` + `FormErrorBanner`.
- **Field layout deliberately stays per-form.** A generic schema-driven renderer would have to encode four different grid shapes, a navy inline bar, and a hero card — that abstraction costs more than it saves.
- `idPrefix` reproduces each form's existing id scheme (`contact-*`, `driver-*`, `affiliate-*`, `hero-*`, `newsletter-*`, and **bare ids on `/quote`** — it has no prefix). Ids feed `label for=`, `aria-describedby`, and focus-on-error, so changing a prefix silently breaks all three.
- The hero form keeps its own success panel rather than using `FormSuccessCard`: it must match the shadow/padding of the card it replaces so the hero doesn't shift on submit.
